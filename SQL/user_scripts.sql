...
SELECT usr.user_id,usr.access_code,usr.active_flag,ld.user_name,st.staff_id ,st.staff_type FROM users usr JOIN login_details ld ON usr.user_id = ld.user_id LEFT JOIN staff st ON st.user_id = usr.user_id WHERE usr.user_id = ?;

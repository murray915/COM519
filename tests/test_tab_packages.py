...
def test_validate_itm_list_valid(tab4):
    """ check input list within checker returns false as failed """
    # item format has to be "ITM-nnn", in string list VALUE,VALUE OR N/A
...
def test_validate_itm_list_invalid(tab4):
    """ check input list within checker returns true as failed """
    # item format has to be "ITM-nnn", in string list VALUE,VALUE OR N/A
...
def test_check_user_inputs_valid(tab4):
...
@patch("tkinter.messagebox.showerror") # added patch to stop popups for mocked window during test
def test_check_user_inputs_missing_variable(mock_msgbox, tab4):
    """ full run of user information checker. Missing var """
    variable_list = ["Name", "", "ITM-001", 1] # missing value
...
def test_check_user_inputs_invalid_active_flag(mock_msgbox, tab4):
    ...
    variable_list = ["Name", "Desc", "ITM-001", 2] # bool not input
...
def test_check_user_inputs_invalid_items_consumed(mock_msgbox, tab4):
    ...
    variable_list = ["Name", "Desc", 123, 1] # item input incorrect
    # call mocked tab4 window
